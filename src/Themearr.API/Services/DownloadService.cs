...
public class DownloadService(Database db, ILogger<DownloadService> log)
{
    ...
    private async Task RunAsync(string movieId, string url)
    {
        try
        {
            ...
            {
                FileName               = "yt-dlp",
                Arguments              = $"-x --audio-format mp3 --audio-quality 0 --no-playlist --no-simulate --extractor-args \"youtube:player_client=android,web\" --print \"%(title)s\" -o \"{outputTemplate}\" \"{url}\"",
