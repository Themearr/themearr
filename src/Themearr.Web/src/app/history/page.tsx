        ...
        <div className="max-w-2xl">
          ...
          <div className="rounded-xl border border-[#1D2939] overflow-hidden">
            {entries.map((entry, i) => (
              <div
                key={entry.id}
                className={`flex items-center gap-4 px-5 py-4 ${i < entries.length - 1 ? 'border-b border-[#1D2939]' : ''}`}
              >
                {/* Icon */}
                <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-[#12B76A]/15">
