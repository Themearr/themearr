...
| `tests/Themearr.API.Tests/` | xUnit. 389 tests. |
...
```bash
npm test                  # vitest, 77 tests, ~5s
npx tsc --noEmit          # ~2s
npm run lint              # eslint, ~2s
npm run build             # tsc --noEmit && vite build
dotnet test tests/Themearr.API.Tests/Themearr.API.Tests.csproj --nologo   # 389 tests, ~7s
```
...
cd tests/e2e && npm test                 # 10 tests, ~4s
```
...
There is **no PR-time CI**: the test steps live in the release workflow and run *after*
the merge that ships. Local verification is the real gate.
